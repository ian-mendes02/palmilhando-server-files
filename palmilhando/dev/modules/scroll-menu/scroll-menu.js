...
const mask = {
    left: 'linear-gradient(to right, black 98%, transparent)',
    right: 'linear-gradient(to left, black 98%, rgba(0, 0, 0, 0.5), transparent)',
    center: 'linear-gradient(to right, transparent, black 2%, black 98%, transparent)',
}

disable(scrollButton[0]);
scrollWrapper.style.webkitMaskImage = mask.left;
...
scrollWrapper.addEventListener('scroll', () => {
    ...
    if (scrollWrapper.scrollLeft == 0) {
        ...
        scrollWrapper.style.webkitMaskImage = mask.right;
        disable(scrollButton[1]);
    }
    else {
        scrollWrapper.style.webkitMaskImage = mask.center;
    }
});